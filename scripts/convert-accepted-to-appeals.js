...
const APPEALS_DIR = path.join(__dirname, '..', 'original', 'appeals');
...
async function main() {
  ...
  if (!fs.existsSync(ACCEPTED_DIR)) {
    console.error(`❌ Папка ${ACCEPTED_DIR} не найдена`);
    process.exit(1);
  }
  ...
  const files = fs.readdirSync(ACCEPTED_DIR).filter(f => f.endsWith('.json'));
